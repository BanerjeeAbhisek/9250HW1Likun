...
for (i in seq_along(n_grid)) {
  ...
  for (j in 1:B) {
    # Generate a random sample from Gamma(shape=3, scale=7)
...
For $m=100$, the Monte Carlo estimate is $\widehat{\mathbb{E}}(\lambda_1)=384.71$ with MC standard error $0.99$ using $B=200$ replicates.
For $m=1000$, the estimate is $\widehat{\mathbb{E}}(\lambda_1)=3973.66$ with MC standard error $9.03$ using $B=20$ replicates.
...
Indeed, $4m=400$ when $m=100$ and $4m=4000$ when $m=1000$, which are close to the Monte Carlo means ($384.97$ and $3965.54$). This agreement supports that our simulation is producing reasonable values for the largest eigenvalue and that $\mathbb{E}(\lambda_1)$ scales approximately linearly with $m$.
...
All code was run on the Hellbender HPC cluster at the University of Missouri. The SLURM job script and output file (`Hellbender-12537107.out`) are included with this submission.

### Part (1): Original Code Timing

The original `slow_code.R` was run on Hellbender without modification.

**Elapsed time: 97.744 seconds**

The `Res` vector values are:
```
 [1]   -28.22559    36.64656    15.40451    23.33129   -53.76236   -15.88296
 [7]   -20.22855   104.81337    16.73784    18.17527 -3445.86312   -18.07226
[13]   -16.80578  -111.53035    20.07449    15.92461    55.42290   -23.09551
[19]   -15.42458   -37.37079    27.84286    15.25095    28.62125   -35.95263
[25]   -15.38570   -23.57380    52.20241    15.84278    20.38660   -98.86681
[31]   -16.67174   -18.28089  1148.65104    17.97182    16.87560   119.17716
[37]   -19.92431   -15.96772   -57.19390    22.86622    15.44591    38.12727
[43]   -27.47250   -15.25274   -29.03048    35.28720    15.36814    23.82330
[49]   -50.73429   -15.80403   -20.54876    93.56572    16.60747    18.38921
[55]  -689.22662   -17.87387   -16.94734  -127.96050    19.77790    16.01233
[61]    59.08661   -22.64319   -15.46851   -38.91813
```
...
1. **Trigonometric identity**: $\sin^2(d) - \cos^2(d) = -\cos(2d)$. This replaces 4 trig calls per element with 1.

2. **Precomputation**: The value $d = \text{round}(\texttt{vi\_v}[i] \bmod 256)$ depends only on row $i$, not on column $j$ or iteration `iter`. So we precompute $-\cos(2d)$ for all rows once before the loop.

3. **Vectorization**: Instead of the double for-loop, we use `sweep()` to multiply each row of the matrix by its corresponding scalar. This lets R use optimized internal C code.

The optimized code:

```{r eval=FALSE}
# Precompute d_val for each row
d_vals <- round(vi_v %% 256)

# Precompute -cos(2*d) for each row (trig identity)
...
- Optimized elapsed time: **1.576 seconds**
- Speedup over original: **62x**
- Max absolute difference from Part (1): $2.27 \times 10^{-12}$ (essentially zero, due to floating point)
- Results match: **TRUE**
...
| Cores | Elapsed Time (s) | Speedup vs Serial Optimized |
|-------|------------------|-----------------------------|
| 4     | 0.736            | 2.14x                       |
| 8     | 0.770            | 2.05x                       |
| 16    | 0.909            | 1.73x                       |
...
The parallel results also match the original (max difference: $2.27 \times 10^{-12}$).

**Plot of running time vs number of cores:**

```{r echo=FALSE, out.width="80%", fig.align="center"}
knitr::include_graphics("prob5_hellbender/parallel_plot.pdf")
```

The best parallel performance is at 4 cores (2.14x speedup). As we increase cores beyond that, performance actually degrades. At 64 cores, the parallel version is slower than the serial optimized code. This happens because the optimized serial code is already very fast (1.576 seconds for 64 iterations), so the overhead of creating worker processes, copying the large $1000 \times 2000$ matrix to each worker, and collecting results exceeds the time saved by parallelization.

### SLURM Job Script

```{bash eval=FALSE}
#!/bin/bash
#SBATCH --job-name=9250hw1_
#SBATCH --ntasks=64
#SBATCH --nodes=1
...
module load r/4.4.0
Rscript 9250hw1_problem5_hellbender.R
```

# Problem 6

We consider the data matrix at `Mat.dat`. We need to compute the means of the first and third columns. 
...
X_T = X.T
np.savetxt("Mat_transposed.dat", X_T)
```


Now we read the transposed matrix into R and carry out the necessary calculations in R


```{r}
X_T <- as.matrix(read.table("Mat_transposed.dat"))
mean_col1 <- mean(X_T[1, ])
mean_col3 <- mean(X_T[3, ])
mean_col1
mean_col3
```
...
Now we use python to do the entire calculation without reading the matrix into R

```{python}
import numpy as np
X = np.loadtxt("Mat.dat")
mean_col1_py = np.mean(X[:, 0])
mean_col3_py = np.mean(X[:, 2])
mean_col1_py, mean_col3_py
```


We see that in part 1 we got 0.0447586 and  0.620249 as means for first and third columns respectively. And in part 2 we get 0.0447586 and 0.620249 which are the same values as obtained in part 1. 
